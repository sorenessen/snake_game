...
// + Floating text: when a poop activates, “NICE SHIT!” rises up (only one at a time).
//
...
// Floating text: on poop activation (tail vacates), show one “NICE SHIT!” rising for ~2s.
//
// Kept:
...
struct Game {
    ...
    // Floaters (NICE SHIT!) — only one at a time
    ...
    void maybe_activate_poops() {
        ...
        for (const auto& s : poop_seeds) {
            if (!cell_on_snake(s.r, s.c)) {
                ...
                // Spawn floating “NICE SHIT!” if none currently active, and only once this frame
