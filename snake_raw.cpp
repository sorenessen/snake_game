...
// ----- Title music lifecycle (own process we can stop cleanly) -----
...
static void start_title_music() {
    ...
    struct stat st{};
    if (::stat(TITLE_MUSIC_WAV, &st) != 0 || !S_ISREG(st.st_mode)) return;
    if (g_title_music_pid > 0) return; // already running

    pid_t pid = fork();
    if (pid == 0) {
        // child: exec afplay in foreground (no '&')
...
static void stop_title_music() {
    if (g_title_music_pid > 0) {
        // Try graceful stop
        kill(g_title_music_pid, SIGTERM);
        // Give it a brief moment
        for (int i = 0; i < 10; ++i) {
            if (waitpid(g_title_music_pid, nullptr, WNOHANG) > 0) { g_title_music_pid = -1; return; }
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }
        // Force kill if still alive
        kill(g_title_music_pid, SIGKILL);
        waitpid(g_title_music_pid, nullptr, 0);
        g_title_music_pid = -1;
    }
}
...
// // 🟤 Custom local WAVs inside assets/
// static constexpr const char* TITLE_MUSIC_WAV = "assets/groove.wav";
// static constexpr const char* POOP_WAV = "assets/snake_shit.wav";     // on poop activation (tail vacates)
// static constexpr const char* NOM_WAV  = "assets/nom_nom_nom.wav";    // poop-eating (final pellet)
// static constexpr const char* NASTY_WAV= "assets/nasty.wav";          // poop-eating (final pellet)
// static constexpr const char* GROSS_WAV= "assets/gross.wav";          // poop-eating (final pellet)
...
//     std::cout << "\x1b[2J\x1b[H\x1b[?25l" << std::flush;
// // Prefer local title music if present; otherwise use the system splash sound
// if (file_exists(TITLE_MUSIC_WAV)) play_wav(TITLE_MUSIC_WAV);
// else                              play_system_sound(SPLASH_SOUND);
...
//         // One-time debug to stderr so you can confirm detection.
//         if (eat_sfx.empty()) {
//             std::cerr << "[eat-poop sfx] none found in ./assets (falling back to system sound)\n";
//         } else {
//             std::cerr << "[eat-poop sfx] using:";
