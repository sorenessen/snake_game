...
static void cinematic_splash_and_wait() {
    ...
    play_system_sound(SPLASH_SOUND); // pre-game OK to play immediately
...
struct Game {
    ...
    // Count of GOOD pellets remaining in the current 3-poop group
    int good_poops_left_in_group = 0;
    ...
    Game() {
        ...
        // start rotation at a random index (randomized-in-rotation)
    ...
    // Rotate through [NOM_WAV, EW_WAV, FLOSS_WAV], starting at a random index.
    // If the chosen file doesn't exist, try the next ones; if none exist → system fallback.
    void queue_next_eat_poop_wav() {
        const char* candidates[3] = { NOM_WAV, EW_WAV, FLOSS_WAV };
        for (int tries = 0; tries < 3; ++tries) {
            int idx = (eat_poop_sound_idx + tries) % 3;
            const char* path = candidates[idx];
            if (file_exists(path)) {
                queue_wav(path);
                eat_poop_sound_idx = (idx + 1) % 3; // advance to the next for next pile
                return;
            }
        }
        // none exist → fallback and still advance rotation
        queue_sys(REWARD_SOUND);
        eat_poop_sound_idx = (eat_poop_sound_idx + 1) % 3;
    }
    ...
    void update() {
        ...
        if (consuming) {
            if (--chomp_frames <= 0) {
                ...
                poop_to_drop = 3;
                good_poops_left_in_group = 3; // NEW group starts after eating food
        ...
        if (on_poop) {
            ...
            if (st == PoopState::Good) {
                ...
                // Only on the FINAL good pellet of the current group → play one of the rotating WAVs
                if (good_poops_left_in_group > 0) {
                    good_poops_left_in_group--;
                    if (good_poops_left_in_group == 0) {
...
// // Floating text: on poop activation (tail vacates), show one taunt rising for ~2s.
// //
// // Kept:
...
// // 🟤 Your custom poop WAV inside assets/
// static constexpr const char* POOP_WAV = "assets/snake_shit.wav";
...
//     // Floaters (NICE SHIT! / taunts) — only one at a time
//     vector<FloatText> floats;
...
//         bool spawned_float_this_frame = false; // only one message per frame; also we keep only one globally
...
//                 // 🔊 Play your custom poop sound if present; else fallback to built-in
//                 if (file_exists(POOP_WAV)) play_wav(POOP_WAV);
//                 else play_system_sound(FART_SOUND);

//                 // Spawn floating taunt if none currently active, and only once this frame
//                 if (floats.empty() && !spawned_float_this_frame) {
//                     std::uniform_int_distribution<int> pick(0, TAUNTS_COUNT - 1);
//                     std::string msg = TAUNTS[pick(rng)];
//                     int len = (int)msg.size();
