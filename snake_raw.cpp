...
// - Reward: eating your own poop resets speed to base AND **shrinks the snake by 2**
...
struct Game {
    ...
    // Level-up state
    int level = 1;
    int level_flash = 0;            // frames to blink
    bool level_up_trigger = false;  // notify main() to speed up

    // Reward: eat your own poop → reset speed to base AND shrink
    int reward_flash = 0;           // frames to show reward message
    bool slow_down_trigger = false; // notify main() to reset speed
    int  shrink_amount = 0;         // how many segments were removed (for UI)
    ...
    void update() {
        ...
        if (consuming) {
            if (--chomp_frames <= 0) {
                ...
                if (score % 100 == 0) {
                    level++;
                    level_flash = 12;        // ~1.2s at base speed
                    level_up_trigger = true; // main() speeds up
                    play_system_sound("Hero"); // flourish
                }
        ...
        // **Poop reward**: If the next cell holds poop → consume it for reward
        size_t poop_idx = 0;
        bool eat_poop = has_poop_at(nh, &poop_idx);

        // Standard self-collision check
        if (any_of(snake.begin(), snake.end(),
                   [&](const Point& p){ return p.r == nh.r && p.c == nh.c; })) {
            game_over = true; return;
        }
        ...
        if (eat_poop) {
            // remove that poop item
            poops.erase(poops.begin() + (long)poop_idx);

            // reward: reset speed to base (handled in main)
            slow_down_trigger = true;

            // NEW: shrink the snake by 2 segments (not below length 3)
            int safe_min = 3;
            int desired = 2;
            int can_remove = std::max(0, (int)snake.size() - safe_min);
            int to_remove = std::min(desired, can_remove);
            shrink_amount = to_remove;
            while (to_remove-- > 0 && !snake.empty()) {
                snake.pop_back();
            }

            reward_flash = 10; // brief message
    ...
    void render() const {
        ...
        for (int r = 0; r < ROWS; ++r) {
            for (int i = 0; i < pad; ++i) cout << ' ';
            cout << '|';
            // level-up blink: quick invert of the playfield background
...
// // - Splash: inline PNG if kitty/iTerm2 supports it; otherwise colored ASCII splash
// // - Centered board rendering
// // - Level Up every +100 pts (faster), AND **New**: eating your own poop resets speed to base
...
//     // Reward: eat your own poop → reset speed to base
//     int reward_flash = 0;           // frames to show "Time slowed!"
//     bool slow_down_trigger = false; // notify main() to reset speed
...
//                     level_flash = 12;        // ~1.2s at base speed
//                     level_up_trigger = true; // main() speeds up
//                     play_system_sound("Hero"); // extra flourish
//                 }
...
//         // **NEW**: If the next cell holds poop → consume it for reward
//         size_t poop_idx = 0;
//         bool eat_poop = has_poop_at(nh, &poop_idx);

//         // Standard self-collision check (poop isn't part of snake, so fine)
//         if (any_of(snake.begin(), snake.end(),
//                    [&](const Point& p){ return p.r == nh.r && p.c == nh.c; })) {
//             game_over = true; return;
//         }
...
//         if (eat_poop) {
//             // remove that poop item and reward: reset speed to base
//             poops.erase(poops.begin() + (long)poop_idx);
//             slow_down_trigger = true;
//             reward_flash = 10; // brief message
//             play_system_sound(REWARD_SOUND);
...
//             if (consuming)    status += "   (CHOMP!)";
//             if (poop_to_drop) status += "   (Dropping...)";
//             if (reward_flash > 0) status += "   \x1b[93m(Time slowed!)\x1b[0m";
//             center_line(status);
//         }
...
//                     current_tick = chrono::milliseconds(tick_ms);
//                 }
//                 // **NEW** reward: reset to base speed when poop eaten
//                 if (game.slow_down_trigger) {
//                     game.slow_down_trigger = false;
//                     tick_ms = BASE_TICK_MS;
//                     current_tick = chrono::milliseconds(tick_ms);
